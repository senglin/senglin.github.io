...
- Subject: start with one gitmoji + conventional type, optional scope, then imperative subject.
  ...
  - Example: ✨ feat(parser): add support for quoted keys
...
Type Detection → Gitmoji Mapping
- feat → ✨ (new user-facing feature, API, CLI, UI)
- fix → 🐛 (bug fix correcting incorrect behavior)
- docs → 📝 (documentation-only changes)
- style → 🎨 (formatting, whitespace, missing semicolons; no code behavior changes)
- refactor → ♻️ (code change that neither fixes a bug nor adds a feature)
- perf → ⚡️ (performance improvement)
- test → ✅ (adding or adjusting tests)
- build → 📦️ (build system, packaging, dependencies)
- ci → 👷 (CI configuration or scripts)
- chore → 🧹 (maintenance tasks, cleanup, non-src infra)
- revert → ⏪ (revert a previous commit)
- config-only tweaks can use 🔧 when appropriate; combine with closest conventional type if needed.

Heuristics to Infer Type (from file paths + diff)
- If only tests changed (e.g., tests/, __tests__/, *test* files) → test
- If only docs changed (README, docs/, *.md, *.rst) → docs
- If only formatting/lint changes and no logic change → style
- If performance-significant code paths optimized → perf
- If dependency versions or package/build scripts changed → build
- If CI pipelines changed (e.g., .github/workflows/, azure-pipelines) → ci
- If code behavior added → feat
- If code behavior corrected → fix
- If structural/rename/cleanup without behavior change → refactor or chore (prefer refactor within src)
- If reverting a specific commit → revert
...
Examples
- ✨ feat(router): add optional trailing slash support
...
- ♻️ refactor(parser): simplify tokenization pipeline
...
- ⚡️ perf(cache): reduce cold-start latency by 25%
...
- ✅ test(api): add contract tests for error envelopes

  Covers 4xx/5xx envelopes and pagination error shapes.

- 📝 docs(readme): document environment variables and profiles

  Adds table describing required env vars and their defaults.

- ⏪ revert: restore previous pagination behavior
...
6) If issues provided or inferable, add “Closes #…”.
